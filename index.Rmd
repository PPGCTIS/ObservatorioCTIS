...
summ.AC.LP$AC <- as.factor(abbreviate(abbrev))
summ.AC.LP$LP <- as.factor(summ.AC.LP$LP)
data <- summ.AC.LP
...
# replace NA values by Indefinido
projetos.ac$`Área de Concentração`[is.na(projetos.ac$`Área de Concentração`)] <- "Indefinido"
...
# replace NA values by Indefinido
projetos.lp$`Linha de Pesquisa`[is.na(projetos.lp$`Linha de Pesquisa`)] <- "Indefinido"
...
# replace NA values by Indefinido
projetos.situacao$Situação[is.na(projetos.situacao$Situação)] <- "Indefinido"
...
# replace NA values by Indefinido
projetos.natureza$`Natureza do Projeto de Pesquisa`[is.na(projetos.natureza$`Natureza do Projeto de Pesquisa`)] <- "Indefinido"
...
# plot 5 (total de projetos por tipo, por ano)

# crosstable of projects by Natureza do Projeto de Pesquisa
projetos.tipo <- sucupira %>%
  dplyr::distinct(Ano, `Identificador do Projeto de Pesquisa`, .keep_all = TRUE) %>%
  dplyr::group_by(Ano, `Natureza do Projeto de Pesquisa`) %>%
  dplyr::summarise(n = n())

# replace NA values by Indefinido
projetos.tipo$`Natureza do Projeto de Pesquisa`[is.na(projetos.tipo$`Natureza do Projeto de Pesquisa`)] <- "Indefinido"

# plot area setup
layout.m <- c(1)
source("Scripts/plot-margins.R", local = knitr::knit_global())
# Define colors using RColorBrewer
colors <- RColorBrewer::brewer.pal(n = length(unique(projetos.tipo$`Natureza do Projeto de Pesquisa`)), name = "Set3")
...
# plot 6 (total de projetos por categoria de membro, por ano)
...
# replace NA values by Indefinido
projetos.categoria$`Categoria do Membro do Projeto`[is.na(projetos.categoria$`Categoria do Membro do Projeto`)] <- "Indefinido"
...
plot.6 <- plot.6 %>% plotly::layout(
  title = list(
    text = "Projetos por Categoria de Membro",
    x = 0.5,
    xanchor = 'center',
    font = list(size = 24, color = 'white')
  ),
...
plot.2 <- plot.2 %>% plotly::layout(
  title = list(
    text = "Produção bibliográfica por subtipo",
    x = 0.5,
    xanchor = 'center',
    font = list(size = 24, color = 'white')
  ),
...
plot.3 <- plot.3 %>% plotly::layout(
  title = list(
    text = "Produção técnica por subtipo",
    x = 0.5,
    xanchor = 'center',
    font = list(size = 24, color = 'white')
  ),
