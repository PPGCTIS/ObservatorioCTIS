...
tab <- table(
  metricas_artigos$`Ano da Produção`,
  as.logical(metricas_artigos$is_oa)
)
table.3 <- as.table(t(tab))
table.3 <- table.3[order(rownames(table.3)), , drop = FALSE]
...
# Reshape data for Plotly
data_long <- reshape2::melt(table.3, variable.name = "Acesso", value.name = "Frequência")
data_long <- data.frame(data_long, stringsAsFactors = FALSE)
...
if (sjmisc::is_empty(metricas_artigos$citations)) {
  ...
  # sum colums
  table.4 <- colSums(table.4, na.rm = TRUE)
}
...
# drop Altmetric column from table.2.sum
table.2.sum <- table.2.sum[-1]

# Plot area setup para layout 5x4 (5 linhas e 4 colunas)
layout.m <- matrix(
  c(
    1, 2, 3, 4,
    5, 6, 7, 8,
...
```{r sintese-diversidade, include = FALSE, eval = has.sucupira.files}
# get years
source("Scripts/years-sucupira.R", local = knitr::knit_global())
...
# drop columns
egressos <- egressos %>% dplyr::select("Nível Discente", "Ano", "Idade (anos)", "Gênero", "Raça/Cor", "Portador de Deficiência")

# remove duplicates
egressos <- unique(egressos)
colnames(egressos)[which(names(egressos) == "Nome Discente")] <- "Nome"

# group non available data ("Não declarado", "Não dispõe da informação", "Não declarada") into "Não disponível"
egressos <- egressos %>%
  dplyr::mutate(
    `Raça/Cor` = ifelse(`Raça/Cor` %in% c("Não declarado", "Não dispõe da informação", "Não declarada"), "Não disponível", `Raça/Cor`)
  )

# sort levels of Raça/Cor
egressos$`Raça/Cor` <- factor(
  egressos$`Raça/Cor`,
  levels = c("Amarela", "Branca", "Indígena", "Parda", "Preta", "Não disponível")
)

# generate table of egressos per year
table.1 <- table(egressos$`Nível Discente`, egressos$`Ano`)

# plot 1 (Lineplot of Proporção de Mulheres/Homens entre os titulados por ano)
...
# Reshape data for Plotly
data_long <- dplyr::select(table.2, `Nível Discente`, `Ano`, `Gênero`)
...
# plot 2 (Lineplot of Proporção de Raça/Cor entre os titulados por ano)
...
# generate table of raça/cor per year
table.3 <- egressos %>%
  dplyr::group_by(`Raça/Cor`, `Ano`)

# Reshape data for Plotly
data_long <- dplyr::select(table.3, `Raça/Cor`, `Ano`)

table.4 <- table(data_long$`Raça/Cor`, data_long$Ano)

# normalize table 4 by sum year
table.4 <- prop.table(table.4, margin = 2) * 100

# Reshape data for Plotly
data_long <- reshape2::melt(table.4, varnames = c("Raça/Cor", "Ano"), value.name = "Frequência")
...
plot.2 <- plot.2 %>% plotly::layout(
  title = list(
    text = "Raça/Cor entre os titulados",
    x = 0.5,
    xanchor = 'center',
    font = list(size = '100vw', color = 'white')
  ),
...
# plot 3 (Lineplot of Proporção de Portadores de Deficiência entre os titulados por ano)
...
# generate table of raça/cor per year
table.5 <- egressos %>%
  dplyr::group_by(`Portador de Deficiência`, `Ano`)

# Reshape data for Plotly
data_long <- dplyr::select(table.5, `Portador de Deficiência`, `Ano`)

table.6 <- table(data_long$`Portador de Deficiência`, data_long$Ano)

# normalize table 4 by sum year
table.6 <- prop.table(table.6, margin = 2) * 100

# Reshape data for Plotly
data_long <- reshape2::melt(table.6, varnames = c("Portador de Deficiência", "Ano"), value.name = "Frequência")

plot.3 <- plotly::plot_ly(
  data = data_long,
  x = ~Ano,
  y = ~Frequência,
  color = ~`Portador de Deficiência`,
  type = 'bar',
  colors = colors,
  height = "50vmin"
)
...
plot.3 <- plot.3 %>% plotly::layout(
  title = list(
    text = "Portadores de Deficiência entre os titulados",
    x = 0.5,
    xanchor = 'center',
    font = list(size = '100vw', color = 'white')
  ),
...
summ.table.pessoal <-
  cbind(
    sum(egressos$`Gênero` == "Feminino", na.rm = TRUE),
    sum(egressos$`Gênero` == "Masculino", na.rm = TRUE),
    sum(egressos$`Gênero` == "Não Informado", na.rm = TRUE),
    sum(egressos$`Raça/Cor` == "Amarela", na.rm = TRUE),
    sum(egressos$`Raça/Cor` == "Branca", na.rm = TRUE),
    sum(egressos$`Raça/Cor` == "Indígena", na.rm = TRUE),
    sum(egressos$`Raça/Cor` == "Parda", na.rm = TRUE),
    sum(egressos$`Raça/Cor` == "Preta", na.rm = TRUE),
    sum(egressos$`Raça/Cor` == "Não disponível", na.rm = TRUE),
    sum(egressos$`Portador de Deficiência` == "Não", na.rm = TRUE),
    sum(egressos$`Portador de Deficiência` == "Sim", na.rm = TRUE)
  )

# Ensure all factor levels are set
egressos$`Gênero` <- factor(egressos$`Gênero`, levels = c("Feminino", "Masculino", "Não Informado"))
egressos$`Raça/Cor` <- factor(egressos$`Raça/Cor`, levels = c("Amarela", "Branca", "Indígena", "Parda", "Preta", "Não disponível"))
egressos$`Portador de Deficiência` <- factor(egressos$`Portador de Deficiência`,
                                             levels = c("Não", "Sim"),
                                             labels = c("Não Portador", "Portador de Deficiência"))

# rename Sim/Não em Portador de deficiência para "Portador de Deficiência" e "Não Portador de Deficiência"
egressos$`Portador de Deficiência` <-
  factor(egressos$`Portador de Deficiência`,
         levels = c("Não", "Sim"),
         labels = c("Não Portador", "Portador de Deficiência"))

names(summ.table.pessoal) <-
  c(
    levels(factor(egressos$Gênero)),
    levels(egressos$`Raça/Cor`),
    levels(egressos$`Portador de Deficiência`)
  )

# Renomeando as colunas de acordo com a ordem exata que queremos
names(summ.table.pessoal) <- c(
  "Feminino", "Masculino", "Não Informado", # Gênero
  "Amarela", "Branca", "Indígena", "Parda", "Preta", "Não disponível", # Raça/Cor
  "Não Portador", "Portador de Deficiência" # Portador de Deficiência
)
...
```{r sintese-diversidade-carousel-1, echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = 'asis', eval = has.sucupira.files, out.width = "100%"}
cat('<br>')
cat('<br>')
...
```{r sintese-diversidade-carousel-2, echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = 'asis', eval = has.sucupira.files, out.width = "70%"}
cat('<br>')
cat('<br>')

# show sintese-diversidade-1
knitr::include_graphics("index_files/figure-html/sintese-diversidade-1.png")
cat('\n\n')
source("Scripts/fonte-sucupira.R", local = knitr::knit_global())
```
...
# plot 1 (network of project members)
if(network %>% igraph::vcount() != 0){
  plot(
    network,
    directed = FALSE,
    arrow.mode = 0,
