...
# convert NA values in "Sem classificação"
metricas_artigos$Qualis[is.na(metricas_artigos$Qualis)] <- "Sem classificação"
...
for (i in 1:length(estratos)) {
  qualis.PPG[i] <-
    sum(metricas_artigos$Qualis == estratos[i], na.rm = TRUE) / length(metricas_artigos$Qualis) * 100
}
...
barplot(
  ...
  xlab = "%",
  ylab = "Estrato",
  xlim = c(0, max(qualis.PPG)),
  ylim = c(0, length(unique(qualis.PPG)) + 3),
  col = mycolors[2],
  sub = "Programa",
  cex.names = 0.7,
  xaxt = 'n',
...
title(main = "Distribuição (Qualis, SJR, CiteScore)",
      outer = TRUE,
      cex.main = 1.7)
legend(
  "topleft",
...
summ.table.producao <-
  cbind(
    ...
    paste0(
      formatC(
        100 - round((sum(table.2[match(rownames(table.2), c("A1", "A2", "A3", "A4", "B1", "B2", "B3", "B4", "B5")),]) / sum(table.2) * 100), 0),
        format = "d",
        digits = 0,
        big.mark = "."
      ),
    ...
    paste0(
      formatC(
        sum(table.2[match(rownames(table.2), c("B1", "B2", "B3", "B4", "B5")),]) / sum(table.2) * 100,
        format = "d",
        digits = 0,
        big.mark = "."
      ),
    ...
    paste0(
      formatC(
        sum(table.2[match(rownames(table.2), c("A1", "A2", "A3", "A4")),]) / sum(table.2) * 100,
        format = "d",
        digits = 0,
        big.mark = "."
      ),
...
```{r sintese-producao-carousel, eval = any(has.metricas.doi.ppg, has.metricas.doi.sucupira)}
cat('<br>')
cat('<br>')
...
```{r sintese-visibilidade-carousel, eval = any(has.metricas.doi.ppg, has.metricas.doi.sucupira)}
cat('<br>')
cat('<br>')
...
```{r sintese-posdoc, include = FALSE, eval = has.sucupira.files}
# posdocs
sheet <- "Pós-Doc"
source("Scripts/read-xlsx-sucupira.R", local = knitr::knit_global())
...
# check if posdoc data is empty
has.posdocs <- nrow(posdoc.raw) != 0
...
if (has.posdocs) {
  ...
  posdoc.raw$'Ano Matrícula' <-
    substr(as.Date(posdoc.raw$`Vínculo Início`, format = "%d/%m/%Y"),
           1,
           4)
  ...
  table.2 <- table(posdocs.atuais$Nacionalidade,
                   posdocs.atuais$'Ano Matrícula')
...
```{r sintese-posdoc-carousel, eval = and(has.sucupira.files, has.posdocs)}
cat('<br>')
cat('<br>')
