...
try(egressos <- egressos %>% dplyr::select("Nível Discente", "Data Situação", "Nome do Orientador", "Nome Discente", "País"), silent = TRUE)
...
# periodo min
periodo <- min(as.numeric(format(Sys.Date(), "%Y")), anos)
...
discentes <- discentes %>%
  dplyr::filter(`Situação Discente` %in% c("DESLIGADO", "ABANDONOU"))
...
# drop columns
discentes <- discentes %>% dplyr::select("Nível Discente", "Nome Discente", "Nome do Orientador", "Ano de Matrícula", "Ano de Encerramento", "Tempo em curso", "Situação Discente", "País")
