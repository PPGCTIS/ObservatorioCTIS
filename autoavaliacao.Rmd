...
if (knitr::is_html_output()) {
  ...
  knitr::kable(
    ...
    full_width = TRUE,
    position = "center"
  ) %>% kableExtra::column_spec(column = 1:ncol(table.doc), width = paste0(c(0.05, rep(0.80/(length(quadrienal.vigente)), times = length(quadrienal.vigente)),0.15)*(21-3-3), "cm")) %>%
      kableExtra::column_spec(column = 1, bold = TRUE) %>%
      kableExtra::row_spec(row = 0, bold = TRUE)
} else {
  # use tinytable for LaTeX output
  ...
  tinytable::tt(
    table.doc,
    width = c(0.05, rep(0.80/(length(quadrienal.vigente)), times = length(quadrienal.vigente)),0.15),
    style = "bootstrap",
    align = "l",
