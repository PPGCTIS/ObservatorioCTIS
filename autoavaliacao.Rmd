...
# get financiadores from another Sucupira workshheet
...
# replace NA with Indefinido
sucupira.financiadores.df$`Nome do Financiador`[is.na(sucupira.financiadores.df$`Nome do Financiador`)] <- "Indefinida"
sucupira.financiadores.df$`Natureza do Financiador`[is.na(sucupira.financiadores.df$`Natureza do Financiador`)] <- "Indefinida"
sucupira.financiadores.df$`Financiador Estrangeiro`[is.na(sucupira.financiadores.df$`Financiador Estrangeiro`)] <- "Indefinida"
```
...
table.projetos <- table.projetos[order(table.projetos$`Linha de Pesquisa`),]

# add ID to the dataframe
table.projetos$ID <- seq(1, dim(table.projetos)[1])

# reorder columns to ID first
table.projetos <- dplyr::select(table.projetos, ID, dplyr::everything())
...
linhas <- unique(table.projetos$`Linha de Pesquisa`)
linhas <- linhas[!is.na(linhas)]

# create list of tables
lista.linhas <- list()
for (linha in linhas) {
  lista.linhas[[linha]] <- table.projetos[table.projetos$`Linha de Pesquisa` == linha, ]
}
```
...
for (i in seq_along(linhas)) {
  ...
  caption <- paste0('Projetos Liderados por Docentes Permanentes (Linha de Pesquisa: ', linhas[i], ')')
  ...
  knit_expanded <- paste0(
    ...
    "      position = 'center'\n",
    "    ) %>% \n",
    "    kableExtra::column_spec(column = 1:ncol(lista.linhas[[", i, "]]), width = paste0(c(0.01,0.19,0.15,0.25,0.10,0.15,0.15)*(21-3-3), 'cm')) %>% \n",
    "    kableExtra::column_spec(column = 1, bold = TRUE) %>% \n",
    "    kableExtra::row_spec(row = 0, bold = TRUE)\n",
    "} else {\n",
    "  # use tinytable for LaTeX output\n",
    "  linhas.atual <- data.frame(lista.linhas[[", i, "]], check.names = FALSE)\n",
    "  options(tinytable_print_output = 'latex')\n",
    "  tinytable::tt(\n",
    "    linhas.atual,\n",
    ...
    "  ) %>% \n",
    "    tinytable::theme_tt('multipage', rowhead = 1, latex_float = 'H') %>% \n",
    "    tinytable::style_tt(i = 0, j = 1:ncol(lista.linhas[[", i, "]]), bold = TRUE) %>% \n",
    "    tinytable::style_tt(i = 1:nrow(lista.linhas[[", i, "]]), j = 1, bold = TRUE)\n",
    "}\n",
    "source('Scripts/fonte-sucupira.R', local = knitr::knit_global())\n",
    "```"
  )
