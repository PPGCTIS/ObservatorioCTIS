...
```{r 2.1.1-detalhe, eval = all(exists("lista.kw"), render_document, has.sucupira.files)}
if (!sjmisc::is_empty(lista.kw)) {
  ...
  for (i in 1:length(lista.kw)) {
    ...
    knit_expanded.2 <- paste0(
      "\n\n```{r, eval = all(exists('lista.kw'), render_document, has.sucupira.files), echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = 'asis', fig.pos='H'}\n\n",
      "cat('\\n\n\\n\n<!-- -->\\n\n\\n\n')\n\n",
      "# print tables\n\n",
      "if (knitr::is_html_output()) {\n\n",
      "  # use kable for HTML output\n\n",
...
<!--- knit those table chunk statements --> 
`r if(all(exists("out.2"), try(!sjmisc::is_empty(out.2), silent = TRUE), exists("lista.kw"), render_document, has.sucupira.files)){paste(knitr::knit(text = out.2), collapse = '\n\n')}`
