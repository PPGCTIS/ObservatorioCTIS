...
for (i in 1:length(unique(bancas$"Linha de Pesquisa"))) {
  words[[i]] <- unlist(strsplit(bancas$Título[bancas$"Linha de Pesquisa"[i] == unique(bancas$"Linha de Pesquisa")], " "))
  # to lower case
...
```{r 2.1.1-detalhe,eval = has.sucupira.files, echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = 'asis', fig.pos='H'}
...
if (knitr::is_html_output()) {
  ...
  knitr::kable(
    ...
    booktabs = TRUE,
    linesep = "",
    caption = "Palavras-chave mais frequentes por Linha de Pesquisa"
  ) %>% kableExtra::kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"),
    full_width = TRUE,
    position = "center"
  )  %>% kableExtra::column_spec(column = 1:dim(all.kw)[2], width = paste0(c(0.23, 0.23, 0.15, 0.09, 0.09, 0.05, 0.08, 0.08)*(21-3-3), "cm")) %>%
  kableExtra::column_spec(column = 1, bold = TRUE) %>%
  kableExtra::column_spec(column = 2, bold = TRUE) %>%
  kableExtra::row_spec(row = 0, bold = TRUE)
} else {
  # use tinytable for LaTeX output
  all.kw <- data.frame(all.kw, check.names = FALSE)
  options(tinytable_print_output = "latex")
  tinytable::tt(
    all.kw,
    width = c(0.23, 0.23, 0.15, 0.09, 0.09, 0.05, 0.08, 0.08)*(21-3-3),
    style = "bootstrap",
    align = "l",
    caption = ifelse(knitr::is_html_output(), "", "Palavras-chave mais frequentes por Linha de Pesquisa")
  ) %>%
    tinytable::theme_tt("multipage", rowhead = 1, latex_float = "H") %>%
    tinytable::style_tt(i = 0, j = 1:ncol(all.kw), bold = TRUE) %>%
    tinytable::style_tt(i = 1:nrow(all.kw), j = 1, bold = TRUE) %>%
    tinytable::style_tt(i = 1:nrow(all.kw), j = 2, bold = TRUE)
}
