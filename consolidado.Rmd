...
tb_descritiva %>%
  gtsummary::as_kable_extra(
    ...
    linesep = "",
    caption = paste0(
      "Razão discentes/docentes por ano (",
...
# MODELOS
# Modelo bruto
model_bruto <- lm(razao ~ ano, data = dados_modelo)

# Modelo ajustado por ano
model_poisson <- glm(
  total_discentes ~ ano + offset(log(total_docentes)),
...
if(nrow(dados_modelo) > 1) {
  ...
  tb_merged <-
    ...
    gtsummary::modify_footnote(
      gtsummary::all_stat_cols() ~ "Coeficiente (Intervalo de Confiança de 95%)",
      replace = TRUE
    )
  ...
  tb_merged %>%
    gtsummary::as_kable_extra(
      ...
      linesep = "",
      caption = paste0(
        "Razão discentes/docente por ano, com modelos bruto e ajustado(",
        min(dados_modelo$ano, na.rm = TRUE), "-", max(dados_modelo$ano, na.rm = TRUE),
        ")"
      )
    ) %>%
